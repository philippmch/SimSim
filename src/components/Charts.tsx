...
export function Histogram({world}:{world:World}){
  ...
  </div><table className="sr-only"><caption>Speed distribution data</caption><thead><tr><th>Range</th><th>Creatures</th></tr></thead><tbody>{bins.map((bin,i)=><tr key={i}><td>{bin.lower.toFixed(2)} to {bin.upper.toFixed(2)}</td><td>{bin.count}</td></tr>)}</tbody></table></>
}
...
export function HistoryChart({world}:{world:World}){
  ...
  </div><table className="sr-only"><caption>Generational trait history data</caption><thead><tr><th>Generation</th><th>Population</th><th>Speed</th><th>Size</th><th>Sense</th></tr></thead><tbody>{points.map(p=><tr key={p.generation}><td>{p.generation}</td><td>{p.population}</td><td>{p.avgSpeed??'Unavailable'}</td><td>{p.avgSize??'Unavailable'}</td><td>{p.avgSense??'Unavailable'}</td></tr>)}</tbody></table></>
}
...
export function BehaviorHistory({world}:{world:World}){
  ...
  </div><table className="sr-only"><caption>Behavior gene history data</caption><thead><tr><th>Generation</th><th>Aggression</th><th>Caution</th><th>Exploration</th></tr></thead><tbody>{points.map(p=><tr key={p.generation}><td>{p.generation}</td><td>{p.avgAggression??'Unavailable'}</td><td>{p.avgCaution??'Unavailable'}</td><td>{p.avgExploration??'Unavailable'}</td></tr>)}</tbody></table></>
}
