...
import { BehaviorHistory, buildHistoryTimeline, Histogram, HistoryChart, summarizeDistribution } from './Charts'
...
export function InsightsPanel({ world, requestedGeneration, onSelectGeneration }: InsightsPanelProps) {
  ...
  return <div className="insights">
    ...
    <div className="chart-card history-card"><div className="card-head"><div><h2>Generational history</h2><p>Outcomes → next population; each row has its own scale</p></div>{historyTimeline.length > 0 && <span>Gen {historyStart}–{historyEnd}</span>}</div><HistoryChart world={world} requestedGeneration={requestedGeneration} onSelectGeneration={onSelectGeneration}/></div>
