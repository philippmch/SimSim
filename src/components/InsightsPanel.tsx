...
import { BEHAVIOR_HISTORY_CONTEXT, BehaviorHistory, buildGenerationDelta, buildHistoryTimeline, formatGenerationDelta, Histogram, HistoryChart, resolveTimelineGeneration, summarizeDistribution } from './Charts'
...
export function InsightsPanel({ world, requestedGeneration, onSelectGeneration }: InsightsPanelProps) {
  ...
  return <div className="insights">
    ...
    <div className="chart-card behavior-history-card"><div className="card-head"><div><h2>Behavior history</h2><p>{BEHAVIOR_HISTORY_CONTEXT} Shown on a 0–1 scale.</p></div>{historyTimeline.length > 0 && <span>Gen {historyStart}–{historyEnd}</span>}</div><BehaviorHistory world={world} requestedGeneration={requestedGeneration}/></div>
  </div>
}
