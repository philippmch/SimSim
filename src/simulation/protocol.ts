...
export type WorkerCommand={type:'init'|'reset';config:Config;epoch?:number}|{type:'restore';world:World;epoch?:number}|{type:'play'|'pause'}|{type:'finish';finishId:number}|{type:'step';stepId?:number}|{type:'speed';speed:number}|{type:'inspect';individualId:number|null}|{type:'intervene';kind:InterventionKind;commandId?:number}
