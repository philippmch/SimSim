...
describe('integrated resource and lifecycle mechanics',()=>{
  ...
  it('carries energy, charges admitted reproduction, seeds offspring energy, and reports age death',()=>{const w=advanced({energyRetention:.5,reproductionEnergyCost:35,offspringEnergy:70,maxAge:2,maturityAge:0}),[parent,aged]=w.creatures;Object.assign(parent,{home:true,energy:100,age:0});Object.assign(aged,{home:true,energy:100,age:2});finishGeneration(w);expect(w.ledger[0].outcomes).toMatchObject({survived:1,aged:1});expect(w.ledger[0].birthsImmature).toBe(0);expect(w.lastReport.aged).toBe(1);const adult=w.creatures.find(c=>c.individualId===parent.individualId)!,child=w.creatures.find(c=>c.parentIndividualId===parent.individualId)!;expect(adult.energy).toBe(15);expect(child.energy).toBe(70);expect(child.lineageId).toBe(parent.lineageId)})
  it('records maturity-blocked parents and delays newborn reproduction by two boundaries',()=>{const w=advanced({initialPopulation:1,energyRetention:1,reproductionEnergyCost:10,offspringEnergy:20,mutationRate:0});const founder=w.creatures[0];Object.assign(founder,{home:true,energy:100,age:0});finishGeneration(w);expect(w.ledger[0]).toMatchObject({birthsEligible:0,birthsAdmitted:0,birthsCapped:0,birthsImmature:1});expect(w.creatures).toHaveLength(1);expect(w.creatures[0]).toMatchObject({individualId:founder.individualId,age:1});
    const adult=w.creatures[0];Object.assign(adult,{home:true,energy:100});finishGeneration(w);const newborn=w.creatures.find(c=>c.parentIndividualId===adult.individualId)!;expect(w.ledger[1]).toMatchObject({birthsEligible:1,birthsAdmitted:1,birthsImmature:0});expect(newborn.age).toBe(0);expect(w.creatures).toHaveLength(2)
    for(const creature of w.creatures)Object.assign(creature,{home:true,energy:100});finishGeneration(w);expect(w.ledger[2]).toMatchObject({birthsImmature:1,birthsAdmitted:1});expect(w.creatures.find(c=>c.individualId===newborn.individualId)?.age).toBe(1)
    for(const creature of w.creatures)Object.assign(creature,{home:true,energy:100});finishGeneration(w);expect(w.ledger[3].birthsImmature).toBe(1);expect(w.ledger[3].birthsAdmitted).toBe(2);expect(w.creatures.some(c=>c.parentIndividualId===newborn.individualId)).toBe(true)
  })
