...
export const defaultConfig:Config={ecologyMode:'energy-regrowth',perceptionMode:'realistic',predationMode:'contest',seed:2187,initialPopulation:42,foodPerDay:42,startSpeed:1,startSize:1,startSense:.18,startingEnergy:110,startAggression:.42,startCaution:.58,startExploration:.55,mutationRate:.05,mutationStrength:.1,mutateSpeed:true,mutateSize:true,mutateSense:true,mutateAggression:true,mutateCaution:true,mutateExploration:true,predatorRatio:1.2,moveEnergyFactor:.7,senseEnergyFactor:.32,dayLength:18,acceleration:.11,turnRate:4,memoryDuration:2.8,commitmentDuration:.8,foodPatchCount:4,foodPatchiness:.72,foodPatchSpread:.12,obstacleCount:4,seasonAmplitude:.22,seasonLength:8,environmentResponse:.45,foodTrend:0,founderPhysicalVariation:.04,founderBehaviorVariation:.06,
  ...
  foodEnergy:22,preyEnergy:30,energyRetention:.65,reproductionEnergyCost:35,offspringEnergy:70,maxAge:24,patchCapacity:60,foodRegrowthRate:.18,fieldOfView:220,detectionFalloff:.65,reactionTime:.15,obstacleOcclusion:true,attackCost:4,handlingTime:.45,contestSharpness:4,evasionWeight:1}
...
/** Strict validation for current-version envelopes. Legacy migrations use sanitizeLegacyConfig instead. */
export function validateExactConfig(input:unknown):Config|null{
  if(!input||typeof input!=='object'||Array.isArray(input))return null
  const source=input as Record<string,unknown>,keys=Object.keys(source)
  if(keys.length!==configKeys.length||keys.some(key=>!configKeySet.has(key)))return null
  if(!ecologyModes.has(source.ecologyMode as EcologyMode)||!perceptionModes.has(source.perceptionMode as PerceptionMode)||!predationModes.has(source.predationMode as PredationMode))return null
  for(const key of booleans)if(typeof source[key]!=='boolean')return null
  for(const [key,[min,max,integer]]of Object.entries(CONFIG_NUMERIC_RANGES)as[NumericKey,readonly[number,number,boolean?]][]){const value=source[key];if(typeof value!=='number'||!Number.isFinite(value)||value<min||value>max||(integer&&!Number.isInteger(value)))return null}
...
export function sanitizeLegacyConfig(input:unknown,clonalFounders=false):Config{const source=input&&typeof input==='object'&&!Array.isArray(input)?input:{};return sanitizeConfig({...source,...CLASSIC_MODES,...(clonalFounders?{founderPhysicalVariation:0,founderBehaviorVariation:0}:{})})}
