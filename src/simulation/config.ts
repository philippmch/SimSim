...
/** Fresh experiments opt into the v6 mechanics; legacy imports retain their historical lifecycle and patch-quality rules. */
export const defaultConfig:Config={ecologyMode:'energy-regrowth',perceptionMode:'realistic',predationMode:'contest',seed:2187,initialPopulation:42,foodPerDay:42,startSpeed:1,startSize:1,startSense:.18,startingEnergy:110,startAggression:.42,startCaution:.58,startExploration:.55,mutationRate:.05,mutationStrength:.1,mutateSpeed:true,mutateSize:true,mutateSense:true,mutateAggression:true,mutateCaution:true,mutateExploration:true,predatorRatio:1.2,moveEnergyFactor:.7,senseEnergyFactor:.32,dayLength:18,acceleration:.11,turnRate:4,memoryDuration:2.8,commitmentDuration:.8,foodPatchCount:4,foodPatchiness:.72,foodPatchSpread:.12,obstacleCount:4,seasonAmplitude:.22,seasonLength:8,environmentResponse:.45,foodTrend:0,founderPhysicalVariation:.04,founderBehaviorVariation:.06,
  ...
  foodEnergy:22,preyEnergy:30,energyRetention:.65,reproductionEnergyCost:35,offspringEnergy:70,maxAge:24,maturityAge:1,patchCapacity:60,foodRegrowthRate:.18,patchQualityVariation:.45,fieldOfView:220,detectionFalloff:.65,reactionTime:.15,obstacleOcclusion:true,attackCost:4,handlingTime:.45,contestSharpness:4,evasionWeight:1}
...
/** The exact v5 envelope shape. It added maturity without patch-quality contrast. */
export const LEGACY_V5_CONFIG_KEYS=[...LEGACY_V4_CONFIG_KEYS,'maturityAge'] as const satisfies readonly (keyof Config)[]
export const V6_ONLY_CONFIG_KEYS=['patchQualityVariation'] as const satisfies readonly (keyof Config)[]
...
export function validateLegacyV4Config(input:unknown):Config|null{
  ...
  return{...(source as unknown as Omit<Config,'maturityAge'|'patchQualityVariation'>),maturityAge:0,patchQualityVariation:0}
}
/** Alias emphasizing that a validated v4 config is normalized into the current shape. */
export const migrateLegacyV4Config=validateLegacyV4Config
const legacyV5KeySet=new Set<string>(LEGACY_V5_CONFIG_KEYS)
/** Strictly validate and migrate a complete v5 config without changing its modes or values. */
export function validateLegacyV5Config(input:unknown):Config|null{
  const source=exactConfigRecord(input,LEGACY_V5_CONFIG_KEYS,legacyV5KeySet)
  if(!source||!validateConfigValues(source,LEGACY_V5_CONFIG_KEYS))return null
  return{...(source as unknown as Omit<Config,'patchQualityVariation'>),patchQualityVariation:0}
}
export const migrateLegacyV5Config=validateLegacyV5Config
...
export function sanitizeLegacyConfig(input:unknown,clonalFounders=false):Config{const source=input&&typeof input==='object'&&!Array.isArray(input)?input:{};return sanitizeConfig({...source,...CLASSIC_MODES,maturityAge:0,patchQualityVariation:0,...(clonalFounders?{founderPhysicalVariation:0,founderBehaviorVariation:0}:{})})}
