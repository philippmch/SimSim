...
describe('experiment lab UI helpers', () => {
  ...
  it('maps named presets to explicit constrained treatment fields', () => {
    const draft = defaultExperimentDraft(defaultConfig)
    expect(EXPERIMENT_PRESETS.map(item => item.label)).toEqual(['Drought', 'High movement cost', 'Predation pressure', 'Patch inequality'])
    ...
    expect(applyExperimentPreset('patch-quality', draft, defaultConfig)).toMatchObject({ interventionKey: 'patchQualityVariation', interventionValue: .85 })
  })

  it('exposes ecological metrics and bounded continuous pressure fields without changing presets', () => {
    ...
    expect(INTERVENTION_CONSTRAINTS.filter(item => ['foodRegrowthRate', 'patchQualityVariation', 'attackCost', 'reactionTime', 'reproductionEnergyCost'].includes(item.key)).map(item => item.key)).toEqual([
      'foodRegrowthRate', 'patchQualityVariation', 'attackCost', 'reactionTime', 'reproductionEnergyCost',
    ])
    expect(normalizeInterventionValue('patchQualityVariation', .47)).toBe(.45)
  ...
  it('blocks preset treatments that are identical at their pressure bounds', () => {
    const cases = [
      ...
      ['patch-quality', { ...defaultConfig, patchQualityVariation: 1 }],
    ] as const
    ...
    expect(treatmentNoOpReason(defaultConfig, applyExperimentPreset('patch-quality', defaultExperimentDraft(defaultConfig), defaultConfig))).toBeNull()
  })
  ...
  it('filters mode-inactive pressures and explains defensive no-ops',()=>{
    ...
    expect(keys).not.toContain('foodRegrowthRate');expect(keys).not.toContain('patchQualityVariation');expect(keys).not.toContain('reproductionEnergyCost');expect(keys).not.toContain('attackCost');expect(keys).not.toContain('reactionTime')
    expect(availableInterventionConstraints(defaultConfig).map(item=>item.key)).toEqual(expect.arrayContaining(['foodRegrowthRate','patchQualityVariation','reproductionEnergyCost','attackCost','reactionTime']))
